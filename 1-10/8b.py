...
def viewable_distance_north(row, column):
    my_height = forest[row][column]
    for n in range(row-1, -1, -1):
        if forest[n][column] >= my_height:
            # We're blocked; figure the distance
            return row - n
    # We weren't blocked until the edge of the forest
    return row


def viewable_distance_south(row, column):
    my_height = forest[row][column]
    for n in range(row + 1, len(forest)):
        if forest[n][column] >= my_height:
            return n - row
    return (len(forest) - row) - 1


def viewable_distance_east(row, column):
    my_height = forest[row][column]
    for n in range(column+1, len(forest[row])):
        if forest[row][n] >= my_height:
            return n - column
    return (len(forest[row]) - column) - 1


def viewable_distance_west(row, column):
    my_height = forest[row][column]
...
def compute_visible_distance(row, column):
    return viewable_distance_north(row, column) * \
        viewable_distance_south(row, column) * \
        viewable_distance_east(row, column) * \
        viewable_distance_west(row, column)
...
def survey_forest():
    ...
    for row in range(1, len(forest)-1):
        for column in range(1, len(forest[0])-1):
            visible_distance = compute_visible_distance(row, column)
            if visible_distance > max_visible_distance:
                max_visible_distance = visible_distance
    print(max_visible_distance)
