...
# Leading space makes the priorities correct by shifting the list right.
...
def get_common_group_item(input_file):
    # This routine assumes that the input file has a multiple of 3 lines!
    first_line = input_file.readline()
    if not first_line:
        raise AllDone
    first_set = set(list(first_line.strip()))
    second_set = set(list(input_file.readline().strip()))
    third_set = set(list(input_file.readline().strip()))
    common = first_set.intersection(second_set).intersection(third_set).pop()
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        input_file = open('3test.txt')
    else:
        input_file = open('3input.txt')
    total_priorities = 0
    while True:
        try:
            common = get_common_group_item(input_file)
        except AllDone:
