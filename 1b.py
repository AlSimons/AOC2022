...
class AllDoneException(Exception):
    # args[0] contains the function's return value
    pass


def get_elf_calories(input_file):
    elfs_calories = 0
    while True:
        line = input_file.readline()
        if not line:
            raise AllDoneException(elfs_calories)
        line = line.strip()
        if not line:
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        input_file = open('1test.txt')
    else:
        input_file = open('1input.txt')
    most_calories = [-1, -1, -1]
    keep_going = True
    while keep_going:
        try:
            elfs_calories = get_elf_calories(input_file)
        except AllDoneException as e:
            keep_going = False
            elfs_calories = e.args[0]
        if elfs_calories > most_calories[0]:
            most_calories[2] = most_calories[1]
            most_calories[1] = most_calories[0]
            most_calories[0] = elfs_calories
        elif elfs_calories > most_calories[1]:
            most_calories[2] = most_calories[1]
            most_calories[1] = elfs_calories
        elif elfs_calories > most_calories[2]:
            most_calories[2] = elfs_calories

    # Processed all the input
    total_top_3 = most_calories[0] + most_calories[1] + most_calories[2]
    print(f"Total top 3 calories are {total_top_3}")
