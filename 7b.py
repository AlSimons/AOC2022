...
class Directory(object):
    ...
    def find_smallest_tree_greater_than(self,
                                        needed_size,
                                        current_smallest_size=None):
        if self.total_size < needed_size:
            # We're too small, nothing in our subtree will help.
            return current_smallest_size

        if current_smallest_size is None or \
                self.total_size < current_smallest_size:
            # We are a possible solution
            current_smallest_size = self.total_size

        for child_dir in self.child_dirs:
            current_smallest_size = child_dir.find_smallest_tree_greater_than(
                needed_size,
...
def main():
    ...
    current_used = root_directory.total_size
    available_space = fs_size - current_used
    needed_to_delete = needed_size - available_space

    size_of_directory_to_delete = \
        root_directory.find_smallest_tree_greater_than(needed_to_delete)
