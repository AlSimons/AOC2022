...
However, because he's heard you have significant experience dealing with signal-
based systems, he convinced the other Elves that it would be okay to give you
their one malfunctioning device - surely you'll have no problem fixing it.
...
this case, your subroutine should report the value 7, because the first start-
of-packet marker is complete after 7 characters have been processed.
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        input_file = open('6test.txt')
    else:
        input_file = open('6input.txt')
    data_stream = input_file.readline()

    for i in range(len(data_stream) - 14):
        s = set(data_stream[i:i+14])
        # print(s)
        if len(s) == 14:
            break
    print(i+14)
